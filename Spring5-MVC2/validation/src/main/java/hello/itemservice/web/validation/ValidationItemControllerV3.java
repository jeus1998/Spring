...
public class ValidationItemControllerV3 {
    ...
    /**
     * @InitBinder -> 해당 컨트롤러에만 영향을 준다. 해당 컨트롤러는 자동으로 적용 가능
     * 글로벌 설정은 별도
     */
    @InitBinder
    ...
    /**
     * addItemV1
     * bindingResult model 자동으로 들어간다.
     * bindingResult.addError 2가지 (new FiledError, new ObjectError)
     * FieldError(objectName, filedName, defaultMessage)
     * ObjectError(objectName, defaultMessage)
     * BidingResult 위치는 @ModelAttribute Item item 다음에 와야 한다.
     */
    ...
    /**
     * 목표: 사용자 입력 오류를 화면(form)에 남도록 하자.
     * fieldError(objectName, fieldName, rejectedValue, binding 실패 여부(boolean), codes, arguments, defaultMessage)
    ...
    /**
     * Validator 인터페이스를 구현한 ItemValidator 활용하기
     */
    // @PostMapping("/add")
